"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.deleteUserID = exports.edit = exports.getUserID = exports.getAllUsers = exports.create = void 0;
var database_1 = require("../database");
// CREATE NEW USER FUNCTION
var create = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, name_1, email, newUser, error_1;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 2, , 3]);
                _a = req.body, name_1 = _a.name, email = _a.email;
                return [4 /*yield*/, database_1.prisma.user.create({
                        data: {
                            name: name_1,
                            email: email,
                        },
                    })];
            case 1:
                newUser = _b.sent();
                res.json(newUser);
                return [3 /*break*/, 3];
            case 2:
                error_1 = _b.sent();
                console.log(error_1);
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.create = create;
// GET ALL USERS FUNCTION
var getAllUsers = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var allUsers, error_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, database_1.prisma.user.findMany()];
            case 1:
                allUsers = _a.sent();
                res.json(allUsers);
                return [3 /*break*/, 3];
            case 2:
                error_2 = _a.sent();
                console.log(error_2);
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.getAllUsers = getAllUsers;
// GET USER BY ID FUNCTION
var getUserID = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, user, error_3;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                id = req.params.id;
                return [4 /*yield*/, database_1.prisma.user.findFirst({
                        where: { id: Number(id) },
                        include: {
                            permission: true,
                        },
                    })];
            case 1:
                user = _a.sent();
                if (!user) {
                    return [2 /*return*/, res.json({
                            error: true,
                            message: "Error: User Not Found",
                        })];
                }
                return [2 /*return*/, res.json({
                        user: user,
                    })];
            case 2:
                error_3 = _a.sent();
                console.log(error_3);
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.getUserID = getUserID;
// UPDATE USER BY ID FUNCTION
var edit = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, _a, name, email, usersWithPermissions, user;
    var _b;
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0:
                id = req.params.id;
                _a = req.body, name = _a.name, email = _a.email;
                return [4 /*yield*/, database_1.prisma.user.findFirst({
                        where: { id: Number(id) },
                        include: {
                            permission: true,
                        },
                    })];
            case 1:
                usersWithPermissions = _c.sent();
                if (!((_b = usersWithPermissions === null || usersWithPermissions === void 0 ? void 0 : usersWithPermissions.permission) === null || _b === void 0 ? void 0 : _b.editpermission)) {
                    // throw new Error("Profile Permition not found!!");
                    return [2 /*return*/, res.json({
                            error: true,
                            message: "Error: Profile Permission Not Found",
                        })];
                }
                if (!usersWithPermissions) {
                    return [2 /*return*/, res.json({
                            error: true,
                            message: "Error: User Not Found",
                        })];
                }
                return [4 /*yield*/, database_1.prisma.user.update({
                        where: {
                            id: Number(req.params.id),
                        },
                        data: {
                            name: name,
                            email: email,
                        },
                    })];
            case 2:
                user = _c.sent();
                return [2 /*return*/, res.json({
                        user: user,
                    })];
        }
    });
}); };
exports.edit = edit;
// DELETE USER BY ID FUNCTION
var deleteUserID = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, error_4;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                id = req.params.id;
                return [4 /*yield*/, database_1.prisma.$transaction([
                        // Deletar a permissão associada primeiro
                        database_1.prisma.permission.deleteMany({
                            where: {
                                userId: Number(id),
                            },
                        }),
                        // Deletar o usuário
                        database_1.prisma.user.delete({
                            where: {
                                id: Number(id),
                            },
                        }),
                    ])];
            case 1:
                _a.sent();
                return [2 /*return*/, res.json({
                        success: true,
                        message: "User and associated permission deleted successfully.",
                    })];
            case 2:
                error_4 = _a.sent();
                console.log(error_4);
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.deleteUserID = deleteUserID;
